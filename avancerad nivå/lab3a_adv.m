K0  = 11;
K1  = 0.2;
Y0  = [0.1; tan(deg2rad(46))];   % [y(0); y'(0)]
xspan = [0, 0.5];

opts_work = odeset('RelTol',1e-12, 'AbsTol',1e-13);  % Arbetslösning
opts_ref  = odeset('RelTol',1e-14, 'AbsTol',1e-15);  % Referenslösning

y_work = solve_crane(xspan, Y0, K0, K1, opts_work);
y_ref  = solve_crane(xspan, Y0, K0, K1, opts_ref);

p = 4;  % ode45 ≈ RK4 i global ordning
method_err = abs(y_ref - y_work) / (1 - 2^(-p));

disp(['y(0.5)         = ' num2str(y_work, '%.8f')])
disp(['referensvärde  = ' num2str(y_ref,  '%.8f')])
disp(['metodfel       = ' num2str(method_err, '%.2e')])

function y_end = solve_crane(xspan, Y0, K0, K1, opts)
    odeFun = @(x, Y) [Y(2); -(K0 - K1*x)*Y(1)*(1 + Y(2)^2)^(3/2)];
    [~, Y] = ode45(odeFun, xspan, Y0, opts);
    y_end = Y(end,1);  % Returnera endast y-komponenten i slutpunkten
end

